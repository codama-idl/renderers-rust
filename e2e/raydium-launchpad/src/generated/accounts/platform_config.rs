...
impl PlatformConfig {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, std::io::Error> {
        if data.get(..PLATFORM_CONFIG_DISCRIMINATOR.len())
            != Some(&PLATFORM_CONFIG_DISCRIMINATOR[..])
        {
...
impl anchor_lang::AccountDeserialize for PlatformConfig {
    fn try_deserialize(buf: &mut &[u8]) -> anchor_lang::Result<Self> {
        if buf.len() < PLATFORM_CONFIG_DISCRIMINATOR.len()
            || buf[..PLATFORM_CONFIG_DISCRIMINATOR.len()] != PLATFORM_CONFIG_DISCRIMINATOR[..]
        {
...
impl anchor_lang::Discriminator for PlatformConfig {
    const DISCRIMINATOR: &[u8] = &PLATFORM_CONFIG_DISCRIMINATOR;
}
