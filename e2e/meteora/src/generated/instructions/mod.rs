//! This code was AUTOGENERATED using the codama library.
//! Please DO NOT EDIT THIS FILE, instead use visitors
//! to add features, then rerun codama to update it.
//!
//! <https://github.com/codama-idl/codama>
//!

pub(crate) mod r#add_liquidity;
pub(crate) mod r#add_liquidity2;
pub(crate) mod r#add_liquidity_by_strategy;
pub(crate) mod r#add_liquidity_by_strategy2;
pub(crate) mod r#add_liquidity_by_strategy_one_side;
pub(crate) mod r#add_liquidity_by_weight;
pub(crate) mod r#add_liquidity_by_weight2;
pub(crate) mod r#add_liquidity_one_side;
pub(crate) mod r#add_liquidity_one_side_precise;
pub(crate) mod r#add_liquidity_one_side_precise2;
pub(crate) mod r#cancel_limit_order;
pub(crate) mod r#claim_fee;
pub(crate) mod r#claim_fee2;
pub(crate) mod r#claim_reward;
pub(crate) mod r#claim_reward2;
pub(crate) mod r#close_bin_array;
pub(crate) mod r#close_claim_fee_operator_account;
pub(crate) mod r#close_limit_order_if_empty;
pub(crate) mod r#close_operator_account;
pub(crate) mod r#close_position;
pub(crate) mod r#close_position2;
pub(crate) mod r#close_position_if_empty;
pub(crate) mod r#close_preset_parameter;
pub(crate) mod r#close_preset_parameter2;
pub(crate) mod r#close_token_badge;
pub(crate) mod r#create_operator_account;
pub(crate) mod r#decrease_position_length;
pub(crate) mod r#for_idl_type_generation_do_not_call;
pub(crate) mod r#fund_reward;
pub(crate) mod r#go_to_a_bin;
pub(crate) mod r#increase_oracle_length;
pub(crate) mod r#increase_position_length;
pub(crate) mod r#increase_position_length2;
pub(crate) mod r#initialize_bin_array;
pub(crate) mod r#initialize_bin_array_bitmap_extension;
pub(crate) mod r#initialize_customizable_permissionless_lb_pair;
pub(crate) mod r#initialize_customizable_permissionless_lb_pair2;
pub(crate) mod r#initialize_lb_pair;
pub(crate) mod r#initialize_lb_pair2;
pub(crate) mod r#initialize_permission_lb_pair;
pub(crate) mod r#initialize_position;
pub(crate) mod r#initialize_position2;
pub(crate) mod r#initialize_position_by_operator;
pub(crate) mod r#initialize_position_pda;
pub(crate) mod r#initialize_preset_parameter;
pub(crate) mod r#initialize_reward;
pub(crate) mod r#initialize_token_badge;
pub(crate) mod r#place_limit_order;
pub(crate) mod r#rebalance_liquidity;
pub(crate) mod r#remove_all_liquidity;
pub(crate) mod r#remove_liquidity;
pub(crate) mod r#remove_liquidity2;
pub(crate) mod r#remove_liquidity_by_range;
pub(crate) mod r#remove_liquidity_by_range2;
pub(crate) mod r#set_activation_point;
pub(crate) mod r#set_pair_status;
pub(crate) mod r#set_pair_status_permissionless;
pub(crate) mod r#set_permissionless_operation_bits;
pub(crate) mod r#set_pre_activation_duration;
pub(crate) mod r#set_pre_activation_swap_address;
pub(crate) mod r#swap;
pub(crate) mod r#swap2;
pub(crate) mod r#swap_exact_out;
pub(crate) mod r#swap_exact_out2;
pub(crate) mod r#swap_with_price_impact;
pub(crate) mod r#swap_with_price_impact2;
pub(crate) mod r#update_base_fee_parameters;
pub(crate) mod r#update_dynamic_fee_parameters;
pub(crate) mod r#update_fees_and_reward2;
pub(crate) mod r#update_fees_and_rewards;
pub(crate) mod r#update_position_operator;
pub(crate) mod r#update_reward_duration;
pub(crate) mod r#update_reward_funder;
pub(crate) mod r#withdraw_ineligible_reward;
pub(crate) mod r#withdraw_protocol_fee;
pub(crate) mod r#zap_protocol_fee;

pub use self::r#add_liquidity::*;
pub use self::r#add_liquidity2::*;
pub use self::r#add_liquidity_by_strategy::*;
pub use self::r#add_liquidity_by_strategy2::*;
pub use self::r#add_liquidity_by_strategy_one_side::*;
pub use self::r#add_liquidity_by_weight::*;
pub use self::r#add_liquidity_by_weight2::*;
pub use self::r#add_liquidity_one_side::*;
pub use self::r#add_liquidity_one_side_precise::*;
pub use self::r#add_liquidity_one_side_precise2::*;
pub use self::r#cancel_limit_order::*;
pub use self::r#claim_fee::*;
pub use self::r#claim_fee2::*;
pub use self::r#claim_reward::*;
pub use self::r#claim_reward2::*;
pub use self::r#close_bin_array::*;
pub use self::r#close_claim_fee_operator_account::*;
pub use self::r#close_limit_order_if_empty::*;
pub use self::r#close_operator_account::*;
pub use self::r#close_position::*;
pub use self::r#close_position2::*;
pub use self::r#close_position_if_empty::*;
pub use self::r#close_preset_parameter::*;
pub use self::r#close_preset_parameter2::*;
pub use self::r#close_token_badge::*;
pub use self::r#create_operator_account::*;
pub use self::r#decrease_position_length::*;
pub use self::r#for_idl_type_generation_do_not_call::*;
pub use self::r#fund_reward::*;
pub use self::r#go_to_a_bin::*;
pub use self::r#increase_oracle_length::*;
pub use self::r#increase_position_length::*;
pub use self::r#increase_position_length2::*;
pub use self::r#initialize_bin_array::*;
pub use self::r#initialize_bin_array_bitmap_extension::*;
pub use self::r#initialize_customizable_permissionless_lb_pair::*;
pub use self::r#initialize_customizable_permissionless_lb_pair2::*;
pub use self::r#initialize_lb_pair::*;
pub use self::r#initialize_lb_pair2::*;
pub use self::r#initialize_permission_lb_pair::*;
pub use self::r#initialize_position::*;
pub use self::r#initialize_position2::*;
pub use self::r#initialize_position_by_operator::*;
pub use self::r#initialize_position_pda::*;
pub use self::r#initialize_preset_parameter::*;
pub use self::r#initialize_reward::*;
pub use self::r#initialize_token_badge::*;
pub use self::r#place_limit_order::*;
pub use self::r#rebalance_liquidity::*;
pub use self::r#remove_all_liquidity::*;
pub use self::r#remove_liquidity::*;
pub use self::r#remove_liquidity2::*;
pub use self::r#remove_liquidity_by_range::*;
pub use self::r#remove_liquidity_by_range2::*;
pub use self::r#set_activation_point::*;
pub use self::r#set_pair_status::*;
pub use self::r#set_pair_status_permissionless::*;
pub use self::r#set_permissionless_operation_bits::*;
pub use self::r#set_pre_activation_duration::*;
pub use self::r#set_pre_activation_swap_address::*;
pub use self::r#swap::*;
pub use self::r#swap2::*;
pub use self::r#swap_exact_out::*;
pub use self::r#swap_exact_out2::*;
pub use self::r#swap_with_price_impact::*;
pub use self::r#swap_with_price_impact2::*;
pub use self::r#update_base_fee_parameters::*;
pub use self::r#update_dynamic_fee_parameters::*;
pub use self::r#update_fees_and_reward2::*;
pub use self::r#update_fees_and_rewards::*;
pub use self::r#update_position_operator::*;
pub use self::r#update_reward_duration::*;
pub use self::r#update_reward_funder::*;
pub use self::r#withdraw_ineligible_reward::*;
pub use self::r#withdraw_protocol_fee::*;
pub use self::r#zap_protocol_fee::*;
